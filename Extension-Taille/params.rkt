...
(provide Ov rd ra rc v0 wmax 1/dt Tboid)
...
(define Tboid 20);La taille des boids 
...
(define Msg-Tboid;Le titre de la frequence 
  (new message%
       (parent VPANEL2)
       (label "Taille des boids")
       (min-width 200)))

(define sliderR-Tboid;Le slider de la frequence
  (new slider%
       (parent VPANEL2)
       (label "Tboid :")
       (min-value 2)
       (max-value 50)
       (callback (lambda (slider evt)
                   (set! Tboid (send sliderR-Tboid get-value))))))
